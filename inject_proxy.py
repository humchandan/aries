...
# Add state and useWeb3 destructured variables
# Replace `const { jwtToken, userProfile } = useWeb3();`
useweb3_addition = """
...
# Update fetchData to load transactions and proxy address
fetch_data_update = """
      if (balRes.ok) {
...
# Add proxy functions right before handleSelectService
proxy_functions = """
...
left_column_additions = """
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button variant="outline" size="icon" onClick={() => copyToClipboard(proxyAddress)}>
                        <Edit className="h-4 w-4" /> {/* Should be copy icon ideally, using edit as fallback from lucide */}
                      </Button>
                    </div>
                  </div>
                  <div>
          ...
          {/* Payment Form Card */}
"""
content = content.replace('{/* Payment Form Card */}', left_column_additions)


# Right Column Additions (Balance & Transactions)
right_column_transactions = """
        ...
        <div className="flex flex-col gap-6 lg:col-span-6 xl:col-span-6">
          <Card>
            ...
            <CardContent>
              <div className="bg-muted/50 border rounded-xl p-5 mb-4 text-center">
                <div className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wide mb-2">Available Utility Balance</div>
                <div className="text-3xl font-semibold tracking-tight font-mono">{balance.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
...
# We need to replace the placeholder Right Column - Transactions I added earlier
import re
content = re.sub(
    r'\{\/\* Right Column - Transactions \*\/\}.*?<\/div>.*?<\/div>.*?<\/div>.*?<\/div>.*?<\/div>',
    right_column_transactions.replace('\\', '\\\\'),
    content,
    flags=re.DOTALL
)
