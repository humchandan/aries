...
import { Award, Check, Copy, Crown, Gem, Loader2, Plane, Shield, Star, Trophy, Users } from "lucide-react";
...
function getProgressToNext(selfInvestment: number, directs: number, teamVolume: number, nextTier: any): number {
...
export function CurrentRank() {
  ...
  const currentTierIdx = mlmTiers.findIndex((t) => t.name.toLowerCase() === userRankName.toLowerCase());
  ...
  const currentLevel =
    mlmLevels.find((l) => l.requiredRank?.toLowerCase() === userRankName.toLowerCase()) || mlmLevels[resolvedIdx];
        ...
        <div className="flex flex-col gap-6 xl:col-span-1">
          ...
          <Card className={`border ${style.borderColor} ${style.bgColor}`}>
            ...
            <CardContent className="flex flex-col items-center text-center gap-4">
              <div
                className={`w-20 h-20 rounded-full flex items-center justify-center border-2 ${style.borderColor} bg-black/30`}
              >
                ...
                <div className="w-full grid grid-cols-3 gap-2 text-center text-xs mt-1">
                  <div className="bg-black/20 rounded-lg p-2">
                    <div className="font-bold text-zinc-200">
                      {Number(currentTier.minSelfInvestment).toLocaleString()}
                    </div>
            ...
            <Card>
              <CardHeader className="pb-3">
                ...
                <CardDescription className="text-xs">
                  Level {resolvedIdx + 2} of {mlmTiers.length}
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2 text-xs">
                  <div className="flex justify-between">
                    ...
                    <span
                      className={
                        selfInvestment >= Number(nextTier.minSelfInvestment) ? "text-emerald-400" : "text-zinc-300"
                      }
                    >
                      {selfInvestment.toLocaleString()} / {Number(nextTier.minSelfInvestment).toLocaleString()} ARES
                      {selfInvestment >= Number(nextTier.minSelfInvestment) && (
                  ...
                  <div className="flex justify-between">
                    ...
                    <span
                      className={teamVolume >= Number(nextTier.minTeamVolume) ? "text-emerald-400" : "text-zinc-300"}
                    >
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              <Button className="w-full gap-2" variant="outline" onClick={handleCopyReferral} disabled={!userAddress}>
          ...
          {mlmTiers.length === 0 ? (
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              {mlmTiers.map((tier, idx) => {
                ...
                const matchingLevel =
                  mlmLevels.find((l) => l.requiredRank?.toLowerCase() === tier.name.toLowerCase()) || mlmLevels[idx];
                  ...
                  >
                    {isCurrent && (
                      <div className="absolute -top-2 right-3">
                        <Badge
                          className={`text-[10px] px-2 py-0.5 ${tierStyle.bgColor} ${tierStyle.color} border ${tierStyle.borderColor}`}
                        >
                          Current
                        </Badge>
                      </div>
                    ...
                    <CardContent className="p-4">
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex items-center gap-3">
                          <div
                            className={`w-9 h-9 rounded-lg flex items-center justify-center border ${tierStyle.borderColor} bg-black/20`}
                          >
                        ...
                        <div className="text-right flex-shrink-0">
                          <div
                            className={`text-lg font-bold font-mono ${isAchieved ? tierStyle.color : "text-zinc-600"}`}
                          >
                            {bonus}%
                          </div>
                      ...
                      <div className="mt-3 grid grid-cols-3 gap-1 text-center">
                        <div className="bg-black/20 rounded p-1.5">
                          <div
                            className={`text-[10px] font-mono font-bold ${isAchieved ? "text-zinc-200" : "text-zinc-600"}`}
                          >
                          ...
                          {Array.from({ length: 10 }, (_, i) => i + 1).map((lvlNum) => {
                            ...
                            return (
                              <div
                                ...
                                title={
                                  unlocked && lvlBonus
                                    ? `Level ${lvlNum}: ${Number(lvlBonus.bonus).toFixed(2)}%`
                                    : `Level ${lvlNum}: Locked`
                                }
      ...
      {mlmLevels.length > 0 && mlmTiers.length > 0 && (
        <Card className="overflow-hidden">
          <CardHeader className="pb-3">
            ...
            <CardDescription className="text-xs">
              Shows which rank unlocks each downline commission level and the bonus % earned. Your active levels are
              highlighted.
            </CardDescription>
          </CardHeader>
          <CardContent className="p-0">
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-zinc-800 bg-zinc-950/60">
                    <th className="px-4 py-3 text-left text-zinc-500 font-semibold uppercase tracking-wider whitespace-nowrap">
                      Level
                    </th>
                    <th className="px-4 py-3 text-left text-zinc-500 font-semibold uppercase tracking-wider whitespace-nowrap">
                      Bonus %
                    ...
                    <th className="px-4 py-3 text-left text-zinc-500 font-semibold uppercase tracking-wider">
                      Your Access
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {Array.from({ length: 10 }, (_, i) => i + 1).map((lvlNum) => {
                    ...
                    const requiredTier = mlmTiers.find((t) => t.name.toLowerCase() === lvl.requiredRank?.toLowerCase());
                    const requiredTierIdx = requiredTier ? mlmTiers.indexOf(requiredTier) : -1;
                    const userHasAccess = currentTier && Number(currentTier.unlockedLevels) >= lvlNum;

                    return (
                      <tr
                        key={lvlNum}
                        className={`border-b border-zinc-800/40 transition-colors ${
                          userHasAccess ? "bg-emerald-950/10 hover:bg-emerald-950/20" : "hover:bg-zinc-900/30"
                        }`}
                      >
                        {/* Level */}
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            ...
                            {userHasAccess && <Check className="w-3 h-3 text-emerald-500" />}
                          </div>
                        </td>
            ...
            <div className="flex flex-wrap items-center gap-4 px-4 py-3 border-t border-zinc-800/50 bg-zinc-950/30">
              <div className="flex items-center gap-1.5 text-[10px] text-zinc-500">
                <span
                  className={`inline-flex items-center justify-center w-5 h-5 rounded-full text-[9px] font-bold ${style.bgColor} ${style.color} border ${style.borderColor}`}
                >
                  ✓
                </span>
                Your current rank
              ...
              <div className="flex items-center gap-1.5 text-[10px] text-zinc-500">
                <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-zinc-800/50 text-zinc-400 border border-zinc-700 text-[9px] font-bold">
                  ✓
                </span>
                Lower rank (achieved)
              </div>
