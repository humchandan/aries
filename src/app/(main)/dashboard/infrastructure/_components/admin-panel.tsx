  ...
  return (
    <Collapsible defaultOpen={defaultOpen} className="overflow-hidden rounded-xl border bg-card">
      <CollapsibleTrigger asChild>
        <button className="group flex w-full items-center gap-3 px-5 py-4 text-left transition-colors hover:bg-muted/30">
...
function RequestsSection({ jwtToken }: { jwtToken: string }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {pending.length === 0 ? (
        <div className="rounded-xl border border-zinc-800 border-dashed py-6 text-center text-sm text-zinc-500">
          No pending utility requests
        </div>
      ) : (
        <div className="space-y-3">
          {pending.map((r) => (
            <div key={r.id} className="space-y-3 rounded-xl border border-zinc-800 p-4">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <div className="font-semibold text-sm">{r.serviceName}</div>
                  <div className="text-xs text-zinc-400">{r.categoryName}</div>
                  <div className="mt-1 font-mono text-xs text-zinc-500">{r.userAddress}</div>
                  <div className="mt-1 text-xs text-zinc-400">
                    Amount: <span className="font-semibold text-white">{Number(r.amount).toFixed(2)} ARES</span>
                  </div>
                </div>
                <Badge variant="outline" className="border-amber-600 text-amber-500 text-xs">
                  PENDING
                </Badge>
              </div>
              {selectedId === r.id ? (
      ...
      {done.length > 0 && (
        <>
          <div className="mt-4 font-semibold text-xs text-zinc-500 uppercase tracking-wider">
          ...
          <div className="max-h-60 space-y-2 overflow-y-auto pr-1">
            {done.slice(0, 10).map((r) => (
              <div
                key={r.id}
                className="flex items-center justify-between rounded-lg border border-zinc-800/40 bg-zinc-950/40 p-3 text-xs"
              >
                <div>
                  <span className="font-semibold text-zinc-300">{r.serviceName}</span>
                  <span className="ml-2 text-zinc-500">{r.userAddress.substring(0, 10)}…</span>
                </div>
                <Badge
                  className={
                    r.status === "APPROVED"
...
function MlmConfigSection({ jwtToken }: { jwtToken: string }) {
  ...
  const tierField = (id: number, field: string) =>
    editTier[id]?.[field] ?? tiers.find((t) => t.id === id)?.[field] ?? "";
  const lvlField = (id: number, field: string) =>
    editLevel[id]?.[field] ?? levels.find((l) => l.id === id)?.[field] ?? "";
  ...
  return (
    <div className="space-y-6">
      {/* Tiers */}
      <div>
        <div className="mb-3 font-semibold text-xs text-zinc-500 uppercase tracking-wider">MLM Rank Tiers</div>
        <div className="overflow-x-auto rounded-xl border border-zinc-800">
          <table className="w-full text-xs">
            <thead>
              <tr className="border-zinc-800 border-b bg-zinc-950/60">
                {["Rank", "Min Stake", "Min Directs", "Min Team Vol", "Unlocked Levels", ""].map((h) => (
                  <th
                    key={h}
                    className="whitespace-nowrap px-3 py-2 text-left font-semibold text-zinc-500 uppercase tracking-wider"
                  >
                    {h}
                  </th>
                ))}
            ...
            <tbody>
              {tiers.map((tier) => (
                <tr key={tier.id} className="border-zinc-800/40 border-b hover:bg-zinc-900/30">
                  <td className="whitespace-nowrap px-3 py-2 font-semibold text-zinc-300">{tier.name}</td>
                  <td className="px-3 py-2">
                    <Input
                      value={tierField(tier.id, "minSelfInvestment")}
                      onChange={(e) => setTierField(tier.id, "minSelfInvestment", e.target.value)}
                      className="h-7 w-28 text-xs"
                    />
                  </td>
                  <td className="px-3 py-2">
                    <Input
                      value={tierField(tier.id, "minDirects")}
                      onChange={(e) => setTierField(tier.id, "minDirects", e.target.value)}
                      className="h-7 w-20 text-xs"
                    />
                  </td>
                  <td className="px-3 py-2">
                    <Input
                      value={tierField(tier.id, "minTeamVolume")}
                      onChange={(e) => setTierField(tier.id, "minTeamVolume", e.target.value)}
                      className="h-7 w-28 text-xs"
                    />
                  </td>
                  <td className="px-3 py-2">
                    <Input
                      value={tierField(tier.id, "unlockedLevels")}
                      onChange={(e) => setTierField(tier.id, "unlockedLevels", e.target.value)}
                      className="h-7 w-20 text-xs"
                    />
                  </td>
                  <td className="px-3 py-2">
      ...
      {/* Levels */}
      <div>
        <div className="mb-3 font-semibold text-xs text-zinc-500 uppercase tracking-wider">Downline Level Bonuses</div>
        <div className="overflow-x-auto rounded-xl border border-zinc-800">
          <table className="w-full text-xs">
            <thead>
              <tr className="border-zinc-800 border-b bg-zinc-950/60">
                {["Level", "Bonus %", "Required Rank", ""].map((h) => (
                  <th key={h} className="px-3 py-2 text-left font-semibold text-zinc-500 uppercase tracking-wider">
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {levels.map((lvl) => (
                <tr key={lvl.id} className="border-zinc-800/40 border-b hover:bg-zinc-900/30">
                  <td className="px-3 py-2 font-semibold text-zinc-300">Level {lvl.level}</td>
                  <td className="px-3 py-2">
                    <Input
                      value={lvlField(lvl.id, "bonus")}
                      onChange={(e) => setLvlField(lvl.id, "bonus", e.target.value)}
...
function CatalogSection({ jwtToken }: { jwtToken: string }) {
  ...
  const [newCatIcon, _setNewCatIcon] = React.useState("fa-star");
  ...
  const saveService = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/admin/utility/services", {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${jwtToken}` },
        body: JSON.stringify({
    ...
    try {
      const res = await fetch("/api/admin/utility/services", {
        ...
        body: JSON.stringify({
          ...
          minAmount: editSvcMin,
          maxAmount: editSvcMax,
          customFields: editSvcFields,
        }),
      });
      if (res.ok) {
  ...
  return (
    <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
      {/* Categories */}
      <div className="space-y-3">
        <div className="font-semibold text-xs text-zinc-500 uppercase tracking-wider">Categories</div>
        <div className="space-y-2">
          {categories.map((cat) => (
            <div
              key={cat.id}
              className={`flex cursor-pointer items-center justify-between rounded-lg border p-3 transition-colors ${selectedCat?.id === cat.id ? "border-zinc-600 bg-zinc-800/60" : "border-zinc-800/50 hover:border-zinc-700"}`}
          ...
          <>
            <div className="space-y-2">
              {(selectedCat.services || []).map((svc: any) =>
                editSvcId === svc.id ? (
                  <div key={svc.id} className="space-y-2 rounded-lg border border-zinc-700/60 bg-zinc-900/60 p-3">
                    ...
                    <div className="flex gap-2">
                      <Input
                        value={editSvcMin}
                        onChange={(e) => setEditSvcMin(e.target.value)}
                        placeholder="Min ARES"
                        className="h-8 text-xs"
                      />
                      <Input
                        value={editSvcMax}
                        onChange={(e) => setEditSvcMax(e.target.value)}
                        placeholder="Max ARES"
                        className="h-8 text-xs"
                      />
                    </div>

                    <div className="space-y-2 border-zinc-800 border-t pt-2">
                      <div className="flex items-center justify-between">
                        <div className="font-semibold text-[10px] text-zinc-500 uppercase">Custom Fields</div>
                        <Button size="sm" variant="ghost" onClick={addEditField} className="h-6 px-2 text-xs">
                      ...
                      {editSvcFields.map((f, idx) => (
                        <div
                          key={idx}
                          className="flex flex-col gap-2 rounded border border-zinc-800/40 bg-zinc-950/40 p-2"
                        >
                          <div className="flex items-center gap-2">
                            <Input
                              value={f.label}
                            ...
                            <select
                              value={f.type}
                              onChange={(e) => updateEditField(idx, "type", e.target.value)}
                              className="h-7 rounded border border-zinc-700 bg-zinc-900 px-1 text-xs text-zinc-300"
                            >
                            ...
                            <label className="flex items-center gap-1 text-[10px] text-zinc-400">
                              <input
                                type="checkbox"
                                checked={f.required}
                                onChange={(e) => updateEditField(idx, "required", e.target.checked)}
                              />{" "}
                              Req
                            </label>
                            <Button
                          ...
                          {f.type === "dropdown" && (
                            <Input
                              ...
                              placeholder="Comma-separated options (e.g. Prepaid, Postpaid)"
                  ...
                  <div
                    key={svc.id}
                    className="flex items-start justify-between rounded-lg border border-zinc-800/40 bg-zinc-950/40 p-3 transition-colors hover:border-zinc-700/50"
                  >
                    <div>
                      <div className="font-medium text-sm text-zinc-200">{svc.name}</div>
                      <div className="mt-0.5 text-xs text-zinc-500">{svc.description}</div>
                      <div className="mt-1 text-xs text-zinc-600">
                        {Number(svc.minAmount).toFixed(0)}–{Number(svc.maxAmount).toFixed(0)} ARES
                      </div>
                    </div>
                    <div className="flex gap-1">
                      <Button
                        size="sm"
                        variant="ghost"
                        className="h-6 w-6 p-0 text-zinc-400 hover:text-primary"
            ...
            <div className="space-y-2 pt-2">
              ...
              <div className="space-y-2 border-zinc-800 border-t pt-2">
                <div className="flex items-center justify-between">
                  <div className="font-semibold text-[10px] text-zinc-500 uppercase">
                    Custom Form Fields (User Inputs)
                  </div>
                ...
                {newSvcFields.map((f, idx) => (
                  <div key={idx} className="flex flex-col gap-2 rounded border border-zinc-800/40 bg-zinc-950/40 p-2">
                    <div className="flex items-center gap-2">
                      <Input
                        value={f.label}
                        onChange={(e) => updateField(idx, "label", e.target.value)}
                        placeholder="Field Label (e.g. Phone No)"
                        className="h-7 flex-1 text-xs"
                      />
                      <select
                        value={f.type}
              ...
              <Button size="sm" className="mt-2 w-full gap-1" onClick={saveService} disabled={loading}>
...
function BlockchainSection({ jwtToken, provider }: { jwtToken: string; provider: any }) {
    ...
    (async () => {
      try {
        ...
        if (provider) {
          ...
          const [sig, fee, own] = await Promise.all([
  ...
  const row = (label: string, value: string) => (
    <div className="flex flex-col gap-1 border-zinc-800/40 border-b py-2 sm:flex-row sm:items-center">
      <span className="w-36 shrink-0 font-semibold text-xs text-zinc-500 uppercase">{label}</span>
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-1 rounded-xl border border-zinc-800 p-4">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          <div className="font-semibold text-xs text-zinc-400 uppercase">Update Signer</div>
          <Input
            value={newSigner}
          ...
          <Button size="sm" className="w-full gap-1" onClick={handleSetSigner} disabled={loading || !provider}>
            <Save className="h-3 w-3" /> Set Signer On-Chain
          </Button>
        </div>
        <div className="space-y-2">
          <div className="font-semibold text-xs text-zinc-400 uppercase">Update Fee Recipient</div>
          ...
          <Button size="sm" className="w-full gap-1" onClick={handleSetFeeRecipient} disabled={loading || !provider}>
            <Save className="h-3 w-3" /> Set Fee Recipient On-Chain
          </Button>
        </div>
      </div>
    </div>
...
function MediaSection({ jwtToken }: { jwtToken: string }) {
  ...
  const publish = async () => {
    ...
    try {
      const res = await fetch("/api/admin/utility/media", {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${jwtToken}` },
        body: JSON.stringify({ type, url, title, caption }),
      });
      const data = await res.json();
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div className="space-y-3">
          <div className="font-semibold text-xs text-zinc-500 uppercase">Publish New Media</div>
          <div className="flex gap-2">
          ...
          <Button size="sm" className="w-full gap-1" onClick={publish} disabled={loading}>
            <Plus className="h-3 w-3" /> Publish Media
          </Button>
        </div>
        <div className="space-y-2">
          <div className="font-semibold text-xs text-zinc-500 uppercase">Published ({media.length})</div>
          <div className="max-h-72 space-y-2 overflow-y-auto pr-1">
            {media.length === 0 ? (
              ...
              media.map((m) => (
                <div
                  key={m.id}
                  className="flex items-center justify-between rounded-lg border border-zinc-800/40 bg-zinc-950/40 p-3"
                >
                  <div className="flex min-w-0 items-center gap-2">
                    <ImageIcon className="h-4 w-4 shrink-0 text-zinc-500" />
                    <div className="min-w-0">
                      <div className="truncate font-medium text-xs text-zinc-200">{m.title}</div>
                  ...
                  <Button
                    size="sm"
                    variant="ghost"
                    className="h-6 w-6 shrink-0 p-0 text-red-500 hover:text-red-400"
                    onClick={() => remove(m.id)}
                  >
                    <Trash2 className="h-3 w-3" />
                  </Button>
...
export function AdminPanel() {
  ...
  React.useEffect(() => {
    async function checkAdmin() {
      ...
      try {
        const contract = new ethers.Contract(PortalFactoryArtifact.address, PortalFactoryArtifact.abi, provider);
  ...
  if (!jwtToken || isAdmin === null) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center p-8 text-center">
        <Loader2 className="mb-4 h-10 w-10 animate-spin text-zinc-500" />
  ...
  if (!isAdmin) {
    return (
      <div className="mx-auto flex min-h-[60vh] max-w-md flex-col items-center justify-center p-8 text-center">
        ...
        <h3 className="mb-2 font-bold text-xl">Access Denied</h3>
        <p className="mb-2 text-sm text-zinc-500">You are connected as:</p>
        <p className="mb-4 break-all rounded border border-zinc-800 bg-zinc-900 px-3 py-2 font-mono text-xs text-zinc-400">
          {userAddress}
        </p>
        <p className="text-xs text-zinc-600">
          Only the authorized master custodian address may access the admin panel.
        </p>
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-4 p-6">
      <div className="space-y-1">
        ...
        <p className="text-muted-foreground text-sm">
          Manage platform configuration, MLM rules, utility catalog, and live media
        </p>
      </div>
      ...
      <SectionShell
        ...
        description="Review and approve/reject pending utility service requests"
        defaultOpen
      >
      ...
      <SectionShell
        icon={Settings}
        title="MLM Config Rules"
        description="Configure rank tier thresholds and downline level bonus percentages"
      >
      ...
      <SectionShell
        icon={LayoutGrid}
        title="Utility Services Catalog"
        description="Manage utility service categories and the services offered within each"
      >
      ...
      <SectionShell
        ...
        description="View and update on-chain contract addresses, signer, and fee recipient"
      >
      ...
      <SectionShell
        ...
        description="Publish images and videos shown on the platform's live events section"
      >
      ...
      <SectionShell
        ...
        description="Manage all system users and bans"
        defaultOpen={false}
      >
        <AdminUsers />
      </SectionShell>
