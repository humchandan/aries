  ...
  try {
    ...
    if (accrued < 100.0) {
      return Response.json(
        { error: "You must have at least 100 ARES accrued to withdraw to MetaMask." },
        { status: 400 },
      );
    }
    ...
    if (user?.isBanned) {
      return Response.json(
        { error: "Your account has been restricted. Blockchain withdrawals are disabled." },
        { status: 403 },
      );
    }
    ...
    if (totalDeposited <= 0) {
      return Response.json(
        { error: "Cannot claim rewards without active validation plans on-chain. Please purchase a plan first." },
        { status: 400 },
      );
    }
