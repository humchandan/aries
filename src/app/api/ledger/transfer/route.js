...
export async function POST(request) {
  ...
  try {
    ...
    if (userProfile?.isBanned) {
      return Response.json({ error: "Your account has been restricted. Transfers are disabled." }, { status: 403 });
    }
    ...
    const recipientUser = await prisma.user.findFirst({
      where: {
        OR: [{ walletAddress: recipient.toLowerCase() }, { proxyAddress: recipient.toLowerCase() }],
      },
    });
    ...
    if (senderBalance < parsedAmount) {
      return Response.json(
        { error: `Insufficient utility portal balance. Available: ${senderBalance.toFixed(2)} ARES` },
        { status: 400 },
      );
    }
