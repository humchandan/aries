...
export async function GET(request) {
  ...
  try {
    ...
    const metamaskWithdrawals = claims
      .filter((c) => c.destination === "metamask")
      .reduce((acc, c) => acc + Number(c.grossAmount), 0);
    const utilityWithdrawals = claims
      .filter((c) => c.destination === "utility")
      .reduce((acc, c) => acc + Number(c.grossAmount), 0);
    ...
    return Response.json({
      ...
      user: {
        ...
        metamaskWithdrawals: metamaskWithdrawals,
        utilityWithdrawals: utilityWithdrawals,
...
export async function POST(request) {
  ...
  try {
    ...
    if (user.profileUpdatesRemaining <= 0) {
      return Response.json(
        { error: "No profile updates remaining. You can only update your details 3 times." },
        { status: 400 },
      );
    }
    ...
    const updatedUser = await prisma.user.update({
      ...
      data: {
        ...
        kycStatus:
          (body.aadharFrontUrl || user.aadharFrontUrl) && user.kycStatus !== "Verified" ? "Pending" : user.kycStatus,
      },
    });
...
async function getMlmRankName(selfInvestment, directs, teamVolume) {
  try {
    ...
    for (let i = MLM_TIERS.length - 1; i >= 0; i--) {
      ...
      if (selfInvestment >= tier.minSelfInvestment && directs >= tier.minDirects && teamVolume >= tier.minTeamVolume) {
        return tier.name;
      }
    }
