...
// ── Admin wallet check (client-side) ────────────────────────────────────────
const ADMIN_WALLET = (
  process.env.NEXT_PUBLIC_ADMIN_WALLET || "0x5Ed6484123dA3dec1834CD472E1ca6b53e97c7B6"
).toLowerCase();
function isAdminAddress(addr: string | null): boolean {
...
const switchOrAddNetwork = async (rawProvider: any): Promise<void> => {
  ...
  try {
    ...
    console.warn(
      "wallet_addEthereumChain failed (non-fatal, connection will proceed):",
      addError.code || addError.message || addError,
    );
  }
};
...
export function Web3Provider({ children }: { children: React.ReactNode }) {
  ...
  // Check admin status when user connects
  useEffect(() => {
    async function checkAdmin() {
      ...
      if (fallbackAdmin && fallbackAdmin.toLowerCase() === userAddress.toLowerCase()) {
        setIsAdmin(true);
        return;
      }

      const cacheKey = `isAdmin_${userAddress.toLowerCase()}`;
      ...
      try {
        ...
        const contract = new ethers.Contract(
          PortalFactoryArtifact.default.address,
          PortalFactoryArtifact.default.abi,
          provider,
        );
        const owner = await contract.owner();
        const isOwner = owner.toLowerCase() === userAddress.toLowerCase();
        setIsAdmin(isOwner);
        localStorage.setItem(cacheKey, isOwner ? "true" : "false");
      } catch (err) {
  ...
  const loadProfile = useCallback(async (token: string, addr: string) => {
    ...
    try {
      ...
      } else if (isAdminAddress(addr)) {
        // Admin wallet is not in the users table — inject a synthetic profile
        // so the landing page skips the registration modal.
        setUserProfile({
  ...
  const runAuthFlow = useCallback(
    async (addr: string, ethSigner: ethers.Signer) => {
      try {
